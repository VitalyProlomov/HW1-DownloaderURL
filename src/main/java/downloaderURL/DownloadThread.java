...
public class DownloadThread implements Runnable {
    ...
    DownloadThread(String url, String directoryPath) {
        fileURL = url;
    ...
    public void run() {
        ...
        try {
            ReadableByteChannel byteChannel = Channels.newChannel(new URL(fileURL).openStream());

            String format = fileURL.split("\\.")[fileURL.split("\\.").length - 1];
            FileOutputStream fileOutputStream =
                    new FileOutputStream(directoryPath + File.separatorChar + "new_file_" + number.incrementAndGet() + "." + format);
